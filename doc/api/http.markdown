...
The HTTP interfaces in luvit are designed to support many features
...
In order to support the full spectrum of possible HTTP applications, luvit's
...
And luvit does not check whether Content-Length and the length of the body
...
`response.write()` is called, luvit assumes you're going to be streaming
...
luvit maintains several connections per server to make HTTP requests.
...
* Sending a 'Connection: keep-alive' will notify luvit that the connection to
...
Since most requests are GET requests without bodies, luvit provides this
...
socket to become free the socket is closed. This means that luvit's
...
marked so as to not keep the luvit process running.  However, it is
...
Note: luvit does not check whether Content-Length and the length of the body
which has been transmitted are equal or not.
...
For efficiency reasons, luvit normally buffers the request headers until you
