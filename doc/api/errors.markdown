...
Errors generated by luvit fall into two categories: JavaScript errors and system
...
**Note**: luvit will generate this class of error to encapsulate system
...
* `plain-filename.js:line:column`, if the frame represents a call internal to luvit.
...
luvit will generate and throw RangeError instances *immediately* -- they are a form
...
luvit will generate and throw TypeError instances *immediately* -- they are a form
of argument validation.
...
`Error`, all exceptions thrown by luvit or the JavaScript runtime *will* be instances of Error.
...
In luvit, system errors are represented as augmented Error objects -- not full
...
writing a luvit program. An exhaustive list may be found [here](http://man7.org/linux/man-pages/man3/errno.3.html).
...
that will run the luvit process.
...
All luvit APIs will treat invalid arguments as exceptional -- that is, if passed
...
provided by luvit -- even user created event emitters and streams will throw
