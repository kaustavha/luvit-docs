...
luvit.  For example a [request to an HTTP
...
Almost all luvit programs, no matter how simple, use Streams in some
way. Here is an example of using Streams in an luvit program:
...
Versions of luvit prior to v0.10 had streams that did not implement the
entire Streams API as it is today.  (See "Compatibility" below for
more information.)

If you are using an older luvit library that emits `'data'` events and
has a [`pause()`][] method that is advisory only, then you can use the
`wrap()` method to create a [Readable][] stream that uses the old stream
as its data source.

You will very rarely ever need to call this function, but it exists
as a convenience for interacting with old luvit programs and libraries.
...
would be piped into the parser, which is a more idiomatic luvit stream
approach.
...
cases in luvit's internals where this is done, particularly in the
...
[tls.CryptoStream][] class, which is deprecated in luvit/io.js v1.0.  If you
find that you have to use `stream.push('')`, please consider another
approach, because it almost certainly indicates that something is
horribly wrong.

### Compatibility with Older luvit Versions

<!--type=misc-->

In versions of luvit prior to v0.10, the Readable stream interface was
...
In luvit v0.10, the Readable class described below was added.
For backwards compatibility with older luvit programs, Readable streams
...
In versions of luvit prior to v0.10, the incoming message data would be
simply discarded.  However, in luvit v0.10 and beyond,
...
No streams in luvit core are object mode streams.  This pattern is only
